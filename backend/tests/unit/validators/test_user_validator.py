...
def test_validate_profile_update_password():
    ...
    with app.test_request_context():
        ...
        result = validate_profile_update({"current_password": "oldpassword", "new_password": "newpassword"})
        assert result is None
        ...
        result = validate_profile_update({"current_password": "oldpassword", "new_password": "short"})
        response, status = result
        assert status == 400
...
def test_valid_data():
    ...
    with app.test_request_context():
        ...
        data = {"name": "John Doe", "email": "john@example.com", "role": "developer", "password": "securepassword"}
