...
def test_github_route_missing_or_malformed_token_returns_401():
    ...
    try:
        ...
        assert (
            "msg" in response_missing.get_json()
            or "message" in response_missing.get_json()
            or "error" in response_missing.get_json()
        )

        # Test malformed authorization header
        response_malformed = client.get(
            "/api/v1/github/auth", headers={"Authorization": "Bearer invalid.token.signature"}
        )
