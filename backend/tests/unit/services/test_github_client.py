...
class TestGitHubClient(unittest.TestCase):
    ...
    def test_get_user_profile_success(self, mock_get):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"login": "testuser", "id": 12345, "name": "Test User"}
    ...
    def test_get_repository(self, mock_get):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"id": 1, "name": "repo1", "full_name": "owner/repo1"}
        mock_get.return_value = mock_response

        repo = self.client.get_repository("owner", "repo1")

        self.assertEqual(repo["name"], "repo1")
        mock_get.assert_called_with("https://api.github.com/repos/owner/repo1", headers=self.client.get_headers())
    ...
    def test_get_repository_issues(self, mock_get):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = [{"id": 1, "number": 101, "title": "Issue 1", "state": "open"}]
        mock_get.return_value = mock_response
