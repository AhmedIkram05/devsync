...
def test_send_to_user_websocket_failure_logs_and_commits(
    mock_db_session, mock_emit, mock_connected_users, notification_data
):
...
def test_send_to_recipients_collects_truthy_results():
    ...
    with patch.object(
        NotificationService, "send_to_user", side_effect=[None, MagicMock(id=2), MagicMock(id=3)]
    ) as mock_send:
...
def test_user_crud_notification_routes_and_excludes_admin(
    action_type, affected_user_role, changed_fields, expected_title, expected_fragment
):
    from src.services.notification_service import NotificationService
...
def test_send_to_project(mock_project_rooms, notification_data):
    ...
    with patch.object(NotificationService, "send_to_user") as mock_send_to_user:
        ...
        NotificationService.send_to_project(
            project_id="project1", notification_type="task", title="Test", message="Test", exclude_user_id="user2"
        )
