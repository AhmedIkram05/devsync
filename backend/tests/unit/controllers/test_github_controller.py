...
class TestGitHubController(unittest.TestCase):
    ...
    def test_github_callback_success(
        ...
        mock_client_instance.get_user_profile.return_value = {"login": "testuser"}
    ...
    @patch("backend.src.api.controllers.github_controller.GitHubClient")
    @patch("backend.src.api.controllers.github_controller.db")
    def test_link_task_with_github(
        self,
        mock_db,
        mock_github_client,
        mock_token_class,
        ...
        mock_request.get_json.return_value = {"repo_id": 1, "issue_number": 42}
    ...
    @patch("backend.src.api.controllers.github_controller.GitHubToken")
    @patch("backend.src.api.controllers.github_controller.User")
    def test_github_callback_missing_parameters(
        self, mock_user_class, mock_token_class, mock_db, mock_oauth_states, mock_github_client
    ):
    ...
    @patch("backend.src.api.controllers.github_controller.GitHubToken")
    @patch("backend.src.api.controllers.github_controller.User")
    def test_github_callback_update_existing_token(
        self, mock_user_class, mock_token_class, mock_db, mock_oauth_states, mock_github_client
    ):
        ...
        mock_client_instance = MagicMock()
        mock_github_client.return_value = mock_client_instance
        mock_client_instance.get_user_profile.return_value = {"login": "updateduser"}
    ...
    def test_github_callback_profile_fetch_failure(
        self, mock_token_class, mock_db, mock_oauth_states, mock_github_client
    ):
    ...
    def test_github_callback_redirection(
        self, mock_current_app, mock_user_class, mock_token_class, mock_db, mock_oauth_states, mock_github_client
    ):
        ...
        mock_current_app.config.get.side_effect = lambda key, default=None: (
            "https://devsync.example.com" if key == "FRONTEND_URL" else default
        )
        ...
        expected_redirect_url = (
            "https://devsync.example.com/github/connected?success=true&github_username=testuser&user_id=1"
        )
        mock_redirect.assert_called_with(expected_redirect_url)
    ...
    @patch("backend.src.api.controllers.github_controller.GitHubToken")
    @patch("backend.src.api.controllers.github_controller.User")
    def test_github_callback_user_not_found(
        self, mock_user_class, mock_token_class, mock_db, mock_oauth_states, mock_github_client
    ):
