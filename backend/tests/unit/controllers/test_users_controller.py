...
def test_update_current_user_password(app, mock_jwt_identity, mock_db, mock_user):
    with (
        app.test_request_context(json={"current_password": "password123", "new_password": "newpassword123"}),
...
def test_update_current_user_wrong_password(app, mock_jwt_identity, mock_db, mock_user):
    with (
        app.test_request_context(json={"current_password": "wrongpassword", "new_password": "newpassword123"}),
        patch("backend.src.api.controllers.users_controller.User.query") as mock_query,
        patch("backend.src.api.controllers.users_controller.verify_password") as mock_verify,
    ):
        # Configure the mocks
...
def test_create_user_success_with_default_role(app, mock_db):
    ...
    with app.test_request_context(json=payload):
        with (
            patch("backend.src.api.controllers.users_controller.User") as mock_user_class,
            ...
            patch(
                "backend.src.services.notification_service.NotificationService.user_crud_notification"
            ) as mock_notify,
        ):
