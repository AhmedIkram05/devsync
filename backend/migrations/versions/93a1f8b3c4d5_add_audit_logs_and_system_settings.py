...
def upgrade():
    ...
    if bind.dialect.name == "postgresql":
        op.create_check_constraint("check_valid_role", "users", "role IN ('developer', 'team_lead', 'admin')")

    # Seed default system settings
    op.bulk_insert(
        system_settings_table,
        [
            {"key": "app_name", "value": "DevSync", "updated_at": datetime.now(UTC)},
            {"key": "allow_registration", "value": True, "updated_at": datetime.now(UTC)},
            {"key": "default_user_role", "value": "developer", "updated_at": datetime.now(UTC)},
            {"key": "github_integration_enabled", "value": True, "updated_at": datetime.now(UTC)},
            {
                "key": "notification_settings",
                "value": {"email_notifications": True, "task_assignments": True, "project_updates": True},
