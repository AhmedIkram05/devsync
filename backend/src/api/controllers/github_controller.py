    ...
    return jsonify(
        {
            ...
            "client_id": current_app.config.get("GITHUB_CLIENT_ID", "")[:4] + "****"
...
def get_github_repositories():
    ...
    if github_ids:
        ...
        logger.info(
            f"[PERF] Database query for existing repos took {db_query_time:.2f}s - found {len(stored_repos)} existing"
        )
    ...
    if repos_needing_activity:
        ...
        logger.info(
            f"[PERF] Concurrent activity fetch took {activity_fetch_time:.2f}s for {len(repos_needing_activity)} repos"
        )
    ...
    for repo in repositories:
        ...
        if include_activity:
            activity_metrics = activity_results.get(
                key,
                {
    ...
    logger.info(
        f"[PERF] Loop processing took {loop_time:.2f}s - processed {len(repositories)} repos, include_activity={include_activity}"
    )
    ...
    return jsonify({"repositories": formatted_response})
        ...
        return jsonify(
            {
                ...
                "repository": {"id": existing_repo.id, "name": existing_repo.repo_name, "url": existing_repo.repo_url},
            }
        ), 409

    # Create new repository record
    new_repo = GitHubRepository(
        repo_name=data["repository_name"], repo_url=data["repository_url"], github_id=repo_data["id"]
    )
...
def get_repository_issues(repo_id):
    ...
    issues = github_client.get_repository_issues(owner=owner, repo=repo_name, state=state, page=page, per_page=per_page)
...
def get_repository_pulls(repo_id):
    ...
    pulls = github_client.get_repository_pulls(owner=owner, repo=repo_name, state=state, page=page, per_page=per_page)
...
def link_task_with_github(task_id):
    ...
    existing_link = TaskGitHubLink.query.filter_by(task_id=task_id, repo_id=data["repo_id"]).first()
